...
OpenLayers.Control.FeaturePopups = OpenLayers.Class(OpenLayers.Control, {
            ...
            this._showPopup = OpenLayers.Function.bindAsEventListener(
                function (evt) {
                    ...
                    if (elem.id) {
                        ...
                        if (ids.length === 3) {
                            this.showFeatureById(ids[1], ids[2]);
                        }
                    }
                },
                this
    ...
    closePopupObj: function (popupObj) {
        ...
        if (popup) {
            ...
            } else if (popup instanceof OpenLayers.Popup){
                if (popup.id) { // If id = null then the popup has been destroyed by another process
                    if (this.map) {
                        this.removeChildren(popup.contentDiv);
                        this.map.removePopup(popup);
                    }
                    popup.destroy(); 
                }
            } else if (typeof popup == 'function'){
                popup({action: "close", type: popupObj.type})
            }
            popupObj.popup = null;
            popupObj.html = "";
        }
