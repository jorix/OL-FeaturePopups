...
OpenLayers.Control.FeaturePopups = OpenLayers.Class(OpenLayers.Control, {
    ...
    unselectGeneric: function() {
        var selControl = this.controls.select;
        if (selControl && this.active) {
            this.unselectingAll = true;
            var layersObjs = this.layersObjs;
            for (var i = 0, len = layersObjs; i < len; i++) {
                var layerObj = layersObjs[i];
                if (!layerObj.popupObjs) {
                    var layer = layerObj.layer,
                        selectedFeatures = layer.selectedFeatures;
                    // layer.selectedFeatures is null after layer is destroyed.
                    if (selectedFeatures) {
                        for (var ii = selectedFeatures.length - 1;
                                                                ii >= 0; ii--) {
                            selControl.unselect(selectedFeatures[ii]);
                        }
                        layerObj.refreshSelection();
                    }
                }
            }
            this.unselectingAll = false;
            this.refreshLayers();
        }
    },
 ...
 */
OpenLayers.Control.FeaturePopups_Utils.createPopupObjs =
                                function(control, popupOptions, popupDefaults) {
...
OpenLayers.Control.FeaturePopups_Utils.showListPopup = function(
    ...
    if (popupObjs.single && selectedFeatures.length === 1 &&
    ...
    if (popupObjs.list && !response) {
        popupObjs.list.showPopup(
            selectedFeatures,
...
/**
 * function: getLocationFromHandler
...
OpenLayers.Control.FeaturePopups.Popup = OpenLayers.Class({
    ...
    showListItem: function(evt) {
        ...
        if (elem.id) {
            ...
            if (ids.length >= 2) {
                var layerObj = this.layerObjs[ids[1]];
                layerObj && layerObj.showSingleFeatureById(ids[2]);
                OpenLayers.Event.stop(evt);
            }
        }
    },
...
OpenLayers.Control.FeaturePopups.Layer = OpenLayers.Class({
     ...
     *
     * May contain two keys: "list" and "single", See <FeaturePopups.Popup>
     *     constructor options for more details for each key.
     *
     ...
     * relatedToClear - ["hover", "hoverList", "single", "listItem"] (internal
     *     use)
     *
     ...
     * relatedToClear: ["hover", "hoverList", "list", "listItem"] (internal use)
     */
    popupOptions: null,
    ...
    initialize: function(control, layer, options) {
        ...
        if (options.popupOptions) {
            ...
            this.popupObjs =
                OpenLayers.Control.FeaturePopups_Utils.createPopupObjs(
                            control, options.popupOptions, defaultPopupOptions);
        }
