...
OpenLayers.Control.FeaturePopups = OpenLayers.Class(OpenLayers.Control, {
     ...
     *     feature, default is <hoverClusterTemplate>.
...
OpenLayers.Control.FeaturePopups.Popup = OpenLayers.Class({
     ...
     *     the entire popup is visible, default is true.
    ...
    observeShowPopup: function(div) {
        for (var i = 0, len = div.childNodes.length; i < len; i++) {
            ...
            if (child.id &&
                OpenLayers.String.startsWith(child.id, 'showPopupId-')
            ) {
    ...
    showListItem: function(evt) {
        var elem = OpenLayers.Event.element(evt),
            layerId = elem.getAttribute("data-showpopup-lid");
        if (layerId) {
            var fid = elem.getAttribute("data-showpopup-fid");
            if (fid) {
                var layerObj = this.control.layerObjs[layerId];
                layerObj && layerObj.showSingleFeatureById(fid, this);
                OpenLayers.Event.stop(evt);
            }
        }
    },
        ...
        if (typeof template == 'string') {
            ...
            template = template.replace(
                ...
                function(a) {
                    _subId++;
                    return (
                        ' id="showPopupId-' + _layerId + '-${id}-' + _subId +
                        '" data-showpopup-lid="' + _layerId +
                        '" data-showpopup-fid="${fid}"'
                    );
                }
            );
            template = template.replace(
