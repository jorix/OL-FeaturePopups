...
OpenLayers.Control.FeaturePopups = OpenLayers.Class(OpenLayers.Control, {
    ...
    /** 
     * Constant: LAYER_EVENT_TYPES 
     * Only required to use <OpenLayers.Control.FeaturePopups> with 2.11 or less
     */ 
    LAYER_EVENT_TYPES: ["featureschanged", "selectionchanged"],
    ...
    destroy: function () {
        ...
        for (var layerId in this.layerObjs) {
            ...
            if (layerObj.eventListeners instanceof Object) {
                layerObj.events.un(layerObj.eventListeners);
            }
            layerObj[layerId].events.destroy();
        }
        this.layerObjs = null;
    ...
    onVisibilitychanged: function (evt) {
        ...
        this.setRefreshPending(this.layerObjs[layer.id]);
        this.refreshLayers();
    },
    ...
    addLayerToControl: function (layer, options) {
        ...
        if (!this.layerObjs[layerId]) {
            ...
            var layerObj = {
                ...
                events: new OpenLayers.Events(this, null, this.LAYER_EVENT_TYPES),
                selection: {},
                selectionHash: "",
                featuresHash: "",
                refreshPendig: null
            };
            if (layerObj.eventListeners) {
                layerObj.events.on(layerObj.eventListeners);
                layerObj.listenFeatures = !!(layerObj.eventListeners && 
                                   layerObj.eventListeners["featureschanged"]);
            }
        ...
        if (layerObj) {
            ...
            if (this.active) {
                if (layer.getVisibility()) {
                    layerObj.events.triggerEvent("featureschanged", {
                        layer: layer,
                        features: []
                    });
                }
    ...
    onBeforefeaturesremoved: function (evt) {
        ...
        if (features.length) {
            var delayRefresh = !this.isEmptyObject(this.layerObjs[layerId].selection);
            if (delayRefresh || layerObj.listenFeatures) {
                ...
                this.delayedRefresh = window.setTimeout(
                    OpenLayers.Function.bind(
                        function() {
                            ...
                            var layerObj = this.layerObjs[layerId];
                            layerObj && this.setRefreshPending(layerObj);
                            this.refreshLayers();
                        }, 
                        this
                    ),
    ...
    onFeaturesadded: function (evt) {
        ...
        this.setRefreshPending(layerObj);
     ...
     */
    setRefreshPending: function (layerObj) {
        if (layerObj.listenFeatures) {
            var featuresHash,
                layer = layerObj.layer,
                layerFeatures = this.getSingleFeatures(layer.features, true);
            // get hash
            if (layer.getVisibility()) {
                var feature,
                    ids = [];
            ...
            // have been changed?
            if (featuresHash !== layerObj.featuresHash) {
                layerObj.refreshPendig = layerFeatures;
                layerObj.featuresHash = featuresHash;
            } else {
                layerObj.refreshPendig = null;
            }
        }
    },
    ...
    addSelectionByIds: function (layerId, featureIds, silent) {
        var layerObj = this.layerObjs[layerId];
        if (layerObj && featureIds.length > 0) {
            var i, len,
                layer = layerObj.layer,
                savedSF = layerObj.selection;
            ...
            this.applySelection(layer, silent);
        }
     },
     ...
     */
    applySelection: function (layer, silent) {
        var i, len,
    ...
    removeSelectionByIds: function (layerId, featureIds, silent) {
        ...
        if (layerObj && featureIds.length > 0) {
            ...
            this.applyDeselection(layer, silent);
        }
     },
     ...
     */
    applyDeselection: function (layer, silent) {
        var i, len,
            savedSF = this.layerObjs[layer.id].selection,
            selectCtl = this.controls.select,
            _indexOf = OpenLayers.Util.indexOf,
    ...
    refreshLayers: function () {
        ...
        var bounds = new OpenLayers.Bounds(),
            ...
            html = [],
            allSelFeatures = [];
        var i, len, layer, layerObj, sFeatures, r;
        for (i=0, len=layers.length; i<len; i++) {
            layer = layers[i];
            layerObj = this.layerObjs[layer.id];
            if (layerObj.refreshPendig) {
                layerObj.events.triggerEvent("featureschanged", {
                    layer: layer,
                    features: layerObj.refreshPendig
                });
                layerObj.refreshPendig = null;
            }
            if (layer.getVisibility()) {
            ...
            if (invalid && !this.silentSelection) {
                layerObj.events.triggerEvent("selectionchanged", {
                    layer: layer,
                    selection: sFeatures
                });
            }
    ...
    getSelectionIds: function (layer){
        ...
        if (this.safeSelection) {
            var id,
                layerObj = this.layerObjs[layer.id],
                savedSF = layerObj? layerObj.selection: {};
            for (id in savedSF) {
                ids.push(id);
            }
        } else {
...
OpenLayers.Control.FeaturePopups.Popup = OpenLayers.Class({
     ...
     * popupType - {String} Type of popup manager: "list", "select", "listItem" 
     *     or "hover"
     * options - {Object} 
     *
     * Valid ptions:
