...
OpenLayers.Control.FeaturePopups = OpenLayers.Class(OpenLayers.Control, {
    ...
    initialize: function (options) {
        ...
        case OpenLayers.Control.FeaturePopups.ACTION_TYPE.RE_SELECT:
    ...
    onFeatureselected: function (evt) {
        ...
        if (!this.refreshSelection && this.safeSelection) {
            var layerId = evt.object.id;
            if (!this.selectionStorage[layerId]) {
                this.selectionStorage[layerId] = {};
            }
            this.addToSelectionStorage(this.selectionStorage[layerId], evt.feature);
        } 
    ...
    onFeatureunselected: function (evt) {
        if (!this.refreshSelection && this.safeSelection) {
            this.removeFromSelectionStorage(
                this.selectionStorage[evt.object.id], 
                evt.feature
            );
        }
        this.showAllSelectedFeatures();
     ...
     *     <refreshAction> is a <OpenLayers.Control.FeaturePopups.ACTION_TYPE> 
     *     SAFE or RE_SELECT.
     *
     * Parameters:
     * evt - {Object} 
        ...
        if (features.length) {
            ...
            if (this.safeSelection) {
                ...
                for (var i = 0 , len = sFeatures.length; i <len; i++) {
                    ...
                    this.addToSelectionStorage(savedSF, feature);
                }
                if (!this.isEmptyObject(savedSF)) {
                    this.selectionStorage[layerId] = savedSF;
                    delayRefresh = true;
                }
            }
            if (delayRefresh) {
    ...
    /**
     * Method: addToSelectionStorage
     *
     * Parameters:
     * savedSF - {Object}
     * feature - 
     */
    addToSelectionStorage: function (savedSF, feature) {
        var fid = feature.fid;
        if (fid) {
            savedSF[fid] = true;
    ...
    /**
     * Method: removeFromSelectionStorage
     *
     * Parameters:
     * savedSF - {Object}
     * feature - 
     */
    removeFromSelectionStorage: function (savedSF, feature) {
        if (!savedSF) { 
            return;
        }
        var fid = feature.fid;
    ...
    onFeaturesadded: function (evt) {
        ...
        if (savedSF) {
            ...
            if (!this.safeSelection) {
                delete this.selectionStorage[layerId];
            }
    ...
    showAllSelectedFeatures: function () {
        ...
        if (!this.safeSelection) {
            this.selectionStorage = {}; // Clear selection storage features
        }
        var layers = this.controls.select.layers || [this.controls.select.layer];
