...
OpenLayers.Control.FeaturePopups = OpenLayers.Class(OpenLayers.Control, {
    ...
    selectFeature: function (feature) {
        if (this.selectBoxStarted) {
            if (!this.multipleCount) {
                this.events.triggerEvent("beforeselectionbox", {});
                this.multipleFirtsFeature = feature;
            }
            this.multipleCount++;
        } else {
            this.destroyHoverPopup();
        ...
        return this.events.triggerEvent("featureselected", {
            feature: feature, multiple: this.multipleCount
        });
    },
    ...
    selectBox: function(position) {
        ...
        this.multipleFirtsFeature = null;
        ...
        // Try select popup
            if (this.showFeatureSelected(this.multipleFirtsFeature)) {
            // If it has been no attempt to show the list.
                return;
            }
        }
        if (this.multipleCount) {
            ...
            for (l=0, len=layers.length; l<len; ++l) {
                ...
                if (sFeatures.length) {
                    html += this.renderLayerTemplate(
                        ...
                        layer,
                        this.renderListFeaturesTemplate(
                            this.templates[layer.id].itemPopupTemplate,
                            sFeatures,
                            bounds
                        )
                    );
            ...
            this.events.triggerEvent("afterselectionbox", {html: html});
        }
    },
    
    /**
     * Method: renderListFeatures
     * Called when the select feature control unselects a feature.
     *
     * Parameters:
     */
    renderListFeaturesTemplate: function (template, features, bounds) {
        var i, len, feature, itemsHtml = "";
        var result = {
            html: "",
            count: 0
        };
        for (i=0, len = features.length; i<len; ++i) {
            ...
            if (feature.cluster && feature.cluster.length) {
                var resultCluster;
                resultCluster = this.renderListFeaturesTemplate(template, feature.cluster, bounds);
     ...
     * Parameters:
     */
    renderLayerTemplate: function (template, layer, itemsResult) {
        if (itemsResult.html && template) {
            return this.renderTemplate(
                template, {
                    name: layer.name, 
                    attribution: layer.attribution, 
                    itemsCount: itemsResult.count, 
                    listLayerItemsHtml: itemsResult.html
                }
            ) + "\n";
        } else {
            return "";
    ...
    showFeatureSelected: function (feature) {
        ...
        if (feature.cluster) {
            if (feature.cluster.length == 1 && template){
                feature = feature.cluster[0];
            } else if (feature.cluster.length == 0){
                return false;
            } else if (feature.cluster.length >= 1) {
                var bounds = new OpenLayers.Bounds();
                var html = this.renderLayerTemplate(
                    this.listLayerItemsTemplate,
                    feature.layer,
                    this.renderListFeaturesTemplate(
                        this.templates[layerId].itemPopupTemplate,
                        feature.cluster,
                        bounds
                    )
                );
