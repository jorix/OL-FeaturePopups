...
OpenLayers.Control.FeaturePopups = OpenLayers.Class(OpenLayers.Control, {
        ...
        if (options.selectOptions !== null) {
            ...
            control.unselectAll = function (options) {
                ...
                if (options && options.except) {
                    ...
                    self.showAllSelectedFeatures("unselectallexcept");
                }
            };
    ...
    onVisibilitychanged: function (evt) {
        this.showAllSelectedFeatures("visibilitychanged");
    },
    ...
    removeLayer: function (layer) {
        ...
        if (layerObjs) {
            ...
            if (this.active) {
                this.showAllSelectedFeatures("removelayer");
    ...
    onFeatureselected: function (evt) {
        ...
        if (!this.selectingSet) {
            this.showAllSelectedFeatures("featureselected");
        }
    },
    ...
    onFeatureunselected: function (evt) {
        ...
        if (this.mode & OpenLayers.Control.FeaturePopups.CLOSE_ON_UNSELECT) {
            this.showAllSelectedFeatures("featureunselected");
        }
    },
                ...
                this.delayedRefresh = window.setTimeout(
                    OpenLayers.Function.bind(
                        function() {
                            ...
                            this.showAllSelectedFeatures("refreshselection-bytimer");
                        }, 
                        this
                    ),
                    this.refreshDelay
    ...
    onFeaturesadded: function (evt) {
        ...
        if (savedSF) {
            ...
            if (this.isEmptyObject(this.delayedRefreshLayers)) {
                this.showAllSelectedFeatures("refreshselection-featuresadded");
            }
        }
    },
    ...
    onFeaturesremoved: function (evt) {
        this.showAllSelectedFeatures("featuresremoved");
    },
    ...
    onSelectBox: function(position) {
        ...
        this.selectingSet = false;
        this.showAllSelectedFeatures("selectbox");
    },
     ...
     */
    showAllSelectedFeatures: function (triggerName) {
        if (this.delayedRefresh !== null) {
            window.clearTimeout(this.delayedRefresh);
            this.delayedRefresh = null;
        }
        ...
        for (i=0, len=layers.length; i<len; i++) {
            ...
            if (layer.visibility) {
                sFeatures = this.getSelection(layer);
                if (sFeatures.length) {
                    allSelFeatures.push({layer:layer, features:sFeatures});
                    r = this.getHtmlLayerSelection(layer, sFeatures, bounds);
                    html.push(r.html);
                    if (r.invalid) {
                        layer.events.triggerEvent("selectionchanged", {
                            layer: layer,
                            triggerName: triggerName, // designed to track logs
                            selection: sFeatures
                        });
                        invalid = true;
                    }
    ...
    /**
     * Function: getHtmlLayerSelection
     */
    getHtmlLayerSelection: function (layer, features, bounds) {
    ...
    /**
     * APIMethod: getSelection
     */
    getSelection: function (layer){
        var sFeatures = [], 
    ...
    /**
     * APIMethod: getSelectionIds
     */
    getSelectionIds: function (layer){
        var i, len, 
            ids = [],
            features = this.getSelection(layer);
        for (i=0, len = features.length; i<len; ++i) {
