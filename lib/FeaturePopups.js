...
OpenLayers.Control.FeaturePopups = OpenLayers.Class(OpenLayers.Control, {
                ...
                for (ii=0, len2 = sFeatures.length; ii<len2; ii++) {
                    ...
                    if (feature.id == featureId) {
                        ...
                        if (template) {
                            ...
                            popupObj.popup = this.showPopup(this.popupSelectClass, 
                                "select", 
                                feature.geometry.getBounds().getCenterLonLat(), 
                                html, 
                                (this.closeBox?"select":"")
                            );
    ...
    showSelectPopup: function (
                     popupObj, popupClass, popupType, lotLat, html, closeBox) {
        ...
        if (popupObj.html !== html) {
            this.destroyAllPopupObjs();
            popupObj.popup = this.showPopup(
                     popupClass, popupType, lotLat, html, (closeBox?"all":""));
            popupObj.html = popupObj.popup ? html : "";
        }
    },
     ...
     * Parameters:
     */
    showPopup: function (popupClass, popupType, lotLat, html, closeMethod) {
        if (!popupClass) {
            return null;
        }
        if (html) {
            if (typeof popupClass == 'string') {
                ...
                if (closeMethod) {
                    ...
                    switch(closeMethod) {
                    case "all": 
                        closeFunct = OpenLayers.Function.bind(
