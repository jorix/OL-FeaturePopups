...
OpenLayers.Control.FeaturePopups = OpenLayers.Class(OpenLayers.Control, {
     ...
     * Parameters:
     */
    observeChildren: function (div) {
        for (var i = 0, len = div.childNodes.length; i < len; i++) {
            var element = div.childNodes[i];
            if (element.id && OpenLayers.String.startsWith(element.id, "showPopup-OpenLayers.Layer.")) {
                OpenLayers.Event.observe(element, "touchend", 
                    OpenLayers.Function.bindAsEventListener(this._showPopup, this));
                OpenLayers.Event.observe(element, "click", 
                    OpenLayers.Function.bindAsEventListener(this._showPopup, this));
            } else {
                this.observeChildren(element);
            }
        }
    },
