...
OpenLayers.Control.FeaturePopups = OpenLayers.Class(OpenLayers.Control, {
     ...
     * {<OpenLayers.Control.FeaturePopups.actionType>} Refresh popups mode when 
     *     some features are removed and re-add, for example when using strategies 
     *     as BBOX. Default is <OpenLayers.Control.FeaturePopups.actionType.AUTO>.
     */
    refreshAction: null,
    ...
    /**
     * Property: multipleSelWorking
     * {Boolean} Internal use only.
     */
    multipleSelWorking: false,
    ...
    initialize: function (options) {
        ...
        options = OpenLayers.Util.extend({
                ...
                refreshAction: OpenLayers.Control.FeaturePopups.actionType.RE_SELECT
            }, 
            options
        );
        ...
        case OpenLayers.Control.FeaturePopups.actionType.RE_SELECT:
            ...
            break;
        case OpenLayers.Control.FeaturePopups.actionType.REMOVE:
     ...
     * Called before some features are removed.
     *
     * Parameters:
     */
    onBeforefeaturesremoved: function (evt) {
        ...
        if (features.length) {
            ...
            var savedSF = {};
            for (var i = 0 , len = sFeatures.length; i <len; i++) {
                var feature = sFeatures[i];
                var fid = feature.fid;
                if (fid) {
                    savedSF[fid] = true;
            ...
            if (!this.isEmptyObject(savedSF)) {
                this.selectSaved[layerId] = savedSF;
     ...
     * Called when some features are added.
     *
     * Parameters:
     */
    onFeaturesadded: function (evt) {
        ...
        var layerId = layer.id;
        var features = evt.features;
        var savedSF = this.selectSaved[layerId];
        if (savedSF) {
            var selectCtl = this.controls.select;
            this.multipleSelWorking = true;
            ...
            for (var i = 0 , len = features.length; i <len; i++) {
                ...
                if (feature.fid && savedSF[feature.fid]) {
                    ...
                    for (var ii = 0 , lenlen = feature.cluster.length; ii <lenlen; ii++) {
                        var cFeature = feature.cluster[ii];
                        if (cFeature.fid && savedSF[cFeature.fid]) {
                            countii++;
                        } else if (savedSF[cFeature.id]) {
                            countii++;
                        }
                    }
                    if (lenlen && countii/lenlen >= 0.666) {
                        selectCtl.select(feature);
                    }
                }
            }
            this.multipleSelWorking = false;
            this.events.triggerEvent("reselectionend", {layer: layer}); 
            delete this.selectSaved[layerId];
            if (this.isEmptyObject(this.selectSaved)) {
                this.showAllSelectedFeatures();
            }
        }
    },
    ...
    showAllSelectedFeatures: function () {
        ...
        this.selectSaved = {}; // Clear save select features
        var layers = this.controls.select.layers || [this.controls.select.layer];
...
/**
 * Enumeration: OpenLayers.Control.FeaturePopups.actionType
 * {Number}
 */
OpenLayers.Control.FeaturePopups.actionType = { 
    NONE: 1,
    RE_SELECT: 2,
    REMOVE: 3 
};