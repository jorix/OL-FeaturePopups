...
OpenLayers.Control.FeaturePopups = OpenLayers.Class(OpenLayers.Control, {
     ...
     *  HOVER_SELECT - Features will be selected by placing the cursor over it.
     *  CLOSE_BOX - Display a close box inside the popups.
     *  UNSELECT_ON_OUT -Unselect when clicking outside any feature.
     *  SELECTION_BOX - Allow feature selection by drawing a box.
     ...
     *     when <mode> contains <OpenLayers.Control.FeaturePopups.SELECTION_BOX>
    ...
    initialize: function (options) {
        ...
        this.popupObjs = {
            list: new OpenLayers.Control.FeaturePopups.Popup(this, "list", this.popupListClass, _closeBox, {
                unselectOnClose: _unselectOnClose,
                observeItems: true,
                clearRelated: function() {
                    var popupObjs = this.control.popupObjs;
                    popupObjs.hover.clear();
                    popupObjs.select.clear();
                    popupObjs.listItem.clear();
                }, 
            }), 
            select: new OpenLayers.Control.FeaturePopups.Popup(this, "select", this.popupSelectClass, _closeBox, {
                unselectOnClose: _unselectOnClose,
                clearRelated: function() {
                    var popupObjs = this.control.popupObjs;
                    popupObjs.hover.clear();
                    popupObjs.list.clear();
                },
            }),
            listItem: new OpenLayers.Control.FeaturePopups.Popup(this, "listItem", this.popupSelectClass, _closeBox, {
            }),
            hover: new OpenLayers.Control.FeaturePopups.Popup(this, "hover", this.popupHoverClass, false, {
        ...
        if (this.popupHoverClass && 
                 !(this.mode & OpenLayers.Control.FeaturePopups.HOVER_SELECT)) {
        ...
        if (this.popupSelectClass) {
            control = new OpenLayers.Control.SelectFeature([], {
                hover: 
                    !!(this.mode & OpenLayers.Control.FeaturePopups.HOVER_SELECT),
                clickout: 
                    !!(this.mode & OpenLayers.Control.FeaturePopups.UNSELECT_ON_OUT),
            ...
            if (this.mode & OpenLayers.Control.FeaturePopups.SELECTION_BOX) {
        ...
        if (this.mode & OpenLayers.Control.FeaturePopups.SAFE_SELECTION) {
    ...
    deactivate: function () {
        if (OpenLayers.Control.prototype.deactivate.apply(this, arguments)) {
            ...
            this.popupObjs.list.clear();
            this.popupObjs.select.clear();
            this.popupObjs.listItem.clear();
            this.popupObjs.hover.clear();
            return true;
        } else {
            return false;
        }
    ...
    addLayer: function (layer, options) {
        ...
        if ((options.hoverTemplate || options.selectTemplate || options.itemTemplate) && 
    ...
    showSelectedFeature: function (feature) {
        ...
        if (feature.cluster) {
            ...
            if (feature.cluster.length >= 1 && response === false) {
                ...
                this.popupObjs.list.showPopup([feature], 
                                                bounds.getCenterLonLat(), html);
        ...
        for (i=0, len=layers.length; i<len; i++) {
            ...
            if (layer.visibility) {
                ...
                if (sFeatures.length) {
                    ...
                    allSelFeatures = allSelFeatures.concat(sFeatures)
                }
            }
        }
        var response = false;
...
/**
 * Constant: HOVER_SELECT
 * {Integer} Used in <mode> indicates that the features will be selected by 
 *     placing the cursor over it.
 */
OpenLayers.Control.FeaturePopups.HOVER_SELECT = 8;

/**
 * Constant: CLOSE_BOX
 * {Integer} Used in <mode> indicates to display a close box inside the popups.
 */
OpenLayers.Control.FeaturePopups.CLOSE_BOX = 16;

/**
 * Constant: UNSELECT_ON_OUT
 * {Integer} Used in <mode> indicates to unselect when clicking outside any 
 *      feature.
 */
OpenLayers.Control.FeaturePopups.UNSELECT_ON_OUT =  32,

/**
 * Constant: SELECTION_BOX
 * {Integer} Used in <mode> indicates to allow feature selection by drawing a box
 */
OpenLayers.Control.FeaturePopups.SELECTION_BOX = 64;
 ...
 *     closed.
 */
OpenLayers.Control.FeaturePopups.UNSELECT_ON_CLOSE = 128;
...
OpenLayers.Control.FeaturePopups.Popup = OpenLayers.Class({
    ...
    /**
     * Property: clearRelated
     * {Function} the function is called to clear the popup related in the 
     *     <control>
     */
    clearRelated: function(){
    },
     ...
     * {Boolean} If true, the popup will follow the cursor (useful for hover)
     */
    followCursor: false,
     ...
     * clearRelated - {Function} Thid function is called to clear the popup 
     *     related in the <control>
     * followCursor - {Boolean} If true, the popup will follow the cursor 
     *     (useful for hover)
     */
    initialize: function (control, type, popupClass, closeBox, options) {
        ...
        if (closeBox) {
            this.onCloseMethod = OpenLayers.Function.bind(
                function(evt) {
     ...
     *
     * Parameters:
     * features - Array({OpenLayers.Feature.Vector}) Features their data appear 
     *     in the popup.
     * lonlat - {<OpenLayers.LonLat>}  The position on the map the popup will
     *     be shown.
     * html - {String} An HTML string to display inside the popup.
     */
    showPopup: function (features, lonLat, html) {
        if (html !== this.html) {
            this.clear();
            this.clearRelated();
            var popupClass = this.popupClass;
            if (popupClass && html) {
                var cont = this.control.events.triggerEvent("beforepopupdisplayed", {
                    type: this.type,
                    features: features,
            ...
            if (this.popup) {
                ...
                this.control.events.triggerEvent("popupdisplayed", {
                    type: this.type,
                    features: features, 
                    div: this.div, 
                    popup: this.popup
    ...
    set: function (lonLat, html) {
        ...
        case "OL":
            ...
            if (this.onCloseMethod) {
                ...
                popup.addCloseBox(this.onCloseMethod);
            ...
            this.html = html;
            this.popup = popup;
            this.followCursor && control.map.events.on(this.moveListener);
            break;
        case "custom":
            var returnObj = this.popupClass(
                           control.map, lonLat, html, this.onCloseMethod, this);
            if (returnObj.div) {
    ...
    clear: function () {
        if (this.popup) {
            ...
            case "OL":
                ...
                this.popup.destroy();
                this.followCursor && control.map.events.un(this.moveListener);
                break;
            case "custom":
                if (this.popup) {
                    if (this.clearCustom) {
