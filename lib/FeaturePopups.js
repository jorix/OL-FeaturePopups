...
OpenLayers.Control.FeaturePopups = OpenLayers.Class(OpenLayers.Control, {
    ...
    initialize: function (options) {
                    ...
                    out: function (feature) {
                        if (feature._lastHighlighter === this.id) {
                            if(feature._prevHighlighter &&
                                ...
                                self.closePopupObj(self.popupObjs.hover);
                            }
                        }
        ...
        if (this.popupSelectClass) {
            ...
            if (this.mode & OpenLayers.Control.FeaturePopups.CLOSE_BOX) {
                this._closeSelect = OpenLayers.Function.bind(
                    function (evt) {
                        if(this.mode & OpenLayers.Control.FeaturePopups.UNSELECT_ON_CLOSE) {
    ...
    onFeaturehighlighted: function (evt) {
        ...
        // Can not detect out feature because was covered by a select popup, 
        //      so must be destroyed.
        // CHK: This seems to happen no more, see trick in the constructor.
        this.closePopupObj(popupObjHover); // Can be removed?
    ...
    showSelectedFeature: function (feature) {
        ...
        if (feature.cluster) {
            ...
            if (template) {
                ...
                this.showFeaturesPopup(this.popupObjs.select, [feature], 
                                              lonLat, html, this._closeSelect);
            }
     ...
     * Parameters:
     */
    showFeaturesPopup: function (popupObj, features, lotLat, html, closeMethod) {
        ...
        if (popupObj.html !== html) {
            this.closeAllPopupObjs();
            this.showPopup(popupObj, features, lotLat, html, closeMethod);
        }
    },
     ...
     * Parameters:
     */
    showPopup: function (popupObj, features, lotLat, html, closeMethod) {
        var popupClass = popupObj.popupClass,
            div = null,
            popup = null;
        if (popupClass && html && 
            false !== this.events.triggerEvent("beforepopupdisplayed", {
                                        type: popupObj.type,
                                        features: features,
