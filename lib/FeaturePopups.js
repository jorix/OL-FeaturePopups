...
OpenLayers.Control.FeaturePopups = OpenLayers.Class(OpenLayers.Control, {
    ...
    initialize: function (options) {
        ...
        this.popupObjs = {
            list: new OpenLayers.Control.FeaturePopups.Popup(this, "list", 
                                              this.popupListClass, _closeBox, {
                unselectOnClose: _unselectOnClose,
                observeItems: true,
                related: ["select"],
                relatedToClear: ["hover", "select", "listItem"]
            }), 
            select: new OpenLayers.Control.FeaturePopups.Popup(this, "select", 
                                            this.popupSelectClass, _closeBox, {
                unselectOnClose: _unselectOnClose,
                related: ["list"],
                relatedToClear: ["hover", "list", "listItem"]
            }),
            listItem: new OpenLayers.Control.FeaturePopups.Popup(this, "listItem", 
                                         this.popupSelectClass, _closeBox, {}),
            hover: new OpenLayers.Control.FeaturePopups.Popup(this, "hover", 
                                                 this.popupHoverClass, false, {
    ...
    addLayer: function (layer, options) {
        this.addLayerToControl(layer, this.getLayerOptions(layer, options));
    },
    ...
    getLayerOptions: function (layer, options) {
        ...
        if (layer instanceof OpenLayers.Layer.Vector) {
            options = OpenLayers.Util.applyDefaults(options, {
                    ...
                    listTemplate: layer.listPopupTemplate || this.layerListTemplate
            });
    ...
    showAllSelectedFeatures: function () {
        ...
        if (allSelFeatures.length === 1 && 
            ...
            var sTemplate = this.templates[selObject.layer.id].selectTemplate;
            if (sTemplate) {
                var lonLat;
        ...
        if (!response) {
            var bounds = new OpenLayers.Bounds(),
                html = [];
            for (i=0, len=allSelFeatures.length; i<len; i++) {
                html.push(this.renderLayerTemplate(allSelFeatures[i], bounds));
            }
     ...
     * Parameters:
     */
    renderLayerTemplate: function (selObject, bounds) {
        var htmlResponse = "",
            layer = selObject.layer,
            layerId = layer.id,
            layerTemplate = this.templates[layerId].listTemplate;
        if (layerTemplate) {
            var i, len, feature,
                html = [],
                features = selObject.features,
                itemTemplate = this.templates[layerId].itemTemplate;
            for (i=0, len = features.length; i<len; ++i) {
                ...
                html.push(this.renderTemplate(itemTemplate, feature));
            }
            if (html.length) {
                htmlResponse = this.renderTemplate(
                    layerTemplate, {
                        layer: layer, 
...
OpenLayers.Control.FeaturePopups.Popup = OpenLayers.Class({
    ...
    clearRelated: function () {
        ...
        for (var i = 0, len = this.relatedToClear.length; i <len; i++) {
            popupObjs[this.relatedToClear[i]].clear();
        }
    },
    ...
    relatedHasHtml : function() {
        ...
        for (var i = 0, len = this.related.length; i <len; i++) {
            if (popupObjs[this.related[i]].html !== "") {
                return true;
            }
        }
        return false;
