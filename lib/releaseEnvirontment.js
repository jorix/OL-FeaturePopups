...
function releaseEnvironment (options) {
    ...
    // private vars
    var iHref = window.location.href.split("?"),
    ...
    if (iHref.length > 1) {
        var regRelease =    /&*release=([\w\.]+)&?.*/i,
            ...
            regLib =        /&*lib=(\w+)&?.*/i,
            res;
        res = regRelease.exec(iHref[1]);
        iRelease = res ? res[1] : "dev";
        iPatch = !!regPatch.exec(iHref[1]);
        iLib = !!regLib.exec(iHref[1]);
    }
    ...
    // set title
    var wrk = document.getElementsByTagName("TITLE")[0],
